...
<template>
  <nav
    class="flex items-center justify-between bg-neutral-900 h-16 px-6 relative"
  >
    <div class="flex-grow">
      <NuxtLink to="/">
    ...
    <div class="md:hidden cursor-pointer z-20" @click="toggleMenu">
      <div class="w-8 h-0.5 bg-gray-200 relative rounded transition-all">
        <span
          class="absolute w-8 h-0.5 bg-gray-200 rounded -top-2 left-0"
        ></span>
        <span
          class="absolute w-8 h-0.5 bg-gray-200 rounded top-2 left-0"
        ></span>
    ...
    <ul
      :class="[
        ...
        isOpen
          ...
          : 'top-[-500px] opacity-0 invisible md:opacity-100 md:visible',
      ]"
      class="space-y-4 md:space-y-0 md:space-x-6 text-center md:text-left text-main font-medium"
    >
      <li class="hover:text-main-hover">
        <NuxtLink to="/">Accueil</NuxtLink>
      </li>
      <li class="hover:text-main-hover">
        <NuxtLink to="/catalogue-de-jeu">Jeux</NuxtLink>
      </li>
      <li v-if="!authStore.user" class="hover:text-main-hover">
