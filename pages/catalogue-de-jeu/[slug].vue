...
const { getScreenshotUrl, formatReleaseDate, transformGameData } =
  useGameUtils();
...
const navigateScreenshot = (d: number) => {
  if (!game.value?.screenshots?.length) {
    return;
  }
...
<template>
  ...
  <div v-if="authStore.user" class="flex flex-wrap gap-5 px-6 py-5">
    <button
      class="bg-main hover:bg-main-hover text-white font-bold py-3 px-5 rounded cursor-pointer"
      v-if="!isGameIsAdd"
      @click="addToUserList"
    >
      Ajouter à ma liste
    ...
    <button
      class="bg-main text-white font-bold py-3 px-5 rounded"
      v-else
  ...
  <div v-if="game?.screenshots?.length" class="max-w-full mx-auto my-8">
    <h3 class="text-white pb-2 border-b border-purple-700 mb-5">
      Captures d'écran de {{ game.name }}
    </h3>

    <div class="flex flex-col gap-4">
      <div
        class="relative w-full h-64 rounded-md shadow-lg cursor-pointer flex items-center justify-center"
      >
      ...
      >
        ...
        >
          ...
          />
          <div
            class="absolute inset-0 bg-purple-700 bg-opacity-20 opacity-0 transition-opacity rounded-sm"
          ></div>
        </div>
      </div>
    </div>
  ...
  <RelatedGamesSection
    :extra-games="game?.expanded_games"
    title="Extensions du jeu"
  />
  <RelatedGamesSection :extra-games="game?.dlcs" title="DLCs" />
  <RelatedGamesSection
    :extra-games="game?.similar_games"
